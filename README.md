...
.venv/bin/python -m unittest discover tests -v   # 回归（必须全绿）
...
| 里程碑 | 功能 |
|--------|------|
| M1 | SQLite 底座 + xlsx/JSON 导入 + 账号管理 |
| M2 | L1 电算评分（Gate/行业/薪资/E1/词典，黄金测试 4 岗位） |
| M3 | BYOK LLM + AI 采集策略 + L2 精评 + 作战卡 |
| M4 | 按计划采集执行器 + 同步刷新（下架/HR 活跃度剔除）+ 简历变更重算 |
| M5 | 招呼语生成/队列/发送（沟通号 UI 级操作 + 全护栏） |
| M6 | 消息中心（DOM 快照轮询 + AI 草稿 + 人工点发） |
| M7 | 模拟面试（出题/追问点评/报告） |
| M9 | 多简历/不可变评分基线/收藏排除/可靠状态模型 |
| M10 | 总览看板/行内 JD/收藏工作台/新前端 |
| M11 | 多简历评分/三版招呼语/平台协同投递 |
| M12 | 模块化采集/精确来源归因/数据分析 |
| M15 | 全模拟前端体验重构/向导/采集工作台/响应式布局 |
| M16 | 前后端真实接入/采集暂停恢复与来源启停/岗位求职阶段 |
| M17 | 采集数据确认删除/页面级 AI 任务队列/流式进度/OFFER 工作流 |
| M18 | 双账号向导/流式采集策略/岗位级进度/标签增强/工作台深链 |
| M19 | 招呼语幂等生成/采集 ETA 学习/评分分析招呼语剩余时间 |
| M20 | 使用指南页（注意事项+顺序教程）/首访引导弹窗/向导跨视图触发 |
