...
| M23 | 无 JD 岗位不参与岗位/匹配度评分（入队剔除+提示） | 本次改动 |
| M24 | 无 JD 岗位不生成工作台分析与招呼语（含自动重算） | 本次改动 |
...
- 没有职位描述（JD）的岗位不参与任何 AI 生成任务（M23 起评分、M24 起工作台分析与招呼语）：批量与单岗请求都由后端剔除并单独回报数量（`skipped_no_jd`），force 也绕不过；简历保存触发的收藏岗位自动分析同样过滤，直连招呼语接口对新生成拒单、对既有结果照常复用。可在采集中心一键补齐 JD 后重新生成
