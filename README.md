...
tests/           379 个单测 + 真实登录态只读 spike 脚本
```
...
- 数据分析支持多标签交叉筛选：经验/学历/行业/规模多选（维度内 OR、维度间 AND），月薪区间按薪资中点(K)过滤，猎头三态可选；候选刻面选项遵循互斥语义（各维度候选项计数不受自身筛选影响、受其他维度影响）。点击图表条目即加入/移除对应筛值。高阶指标含月薪中位数与 P25–P75 分位带、经验/学历/行业 × 平均月薪交叉、投递转化漏斗（工作流阶段 + HR 回复）、公司岗位数 Top10 与技能需求 Top10；页面末尾附「筛选结果岗位」六列明细清单（`job_rows`，综合评分优先、其次月薪中点降序，最多返回 `JOB_ROWS_LIMIT=200` 条）；`/api/analytics` 旧参数调用完全兼容
