...
- 收藏同步依赖推荐页「感兴趣」Tab 的 `job-card-box` 卡片结构（与搜索/公司页同源组件）；若 BOSS 改版解析为空，可跑 `tests/spike_m13_favorites.py`（只读）核对 DOM 后调整解析启发式。同步入库的新岗位默认无 JD，状态条会提示缺失数量并可一键补齐（采集号只读执行）
