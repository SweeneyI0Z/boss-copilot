...
.venv/bin/python -m unittest discover tests -v   # 回归（347 例，必须全绿）
...
tests/           347 个单测 + 真实登录态只读 spike 脚本
```
...
- 没有职位描述（JD）的岗位不参与「岗位评分/匹配度评分」：批量评分时由后端直接剔除并单独回报数量（`skipped_no_jd`），force 也绕不过；可在采集中心一键补齐 JD 后重新评分
