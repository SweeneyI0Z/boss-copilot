...
.venv/bin/python -m unittest discover tests   # 回归（60 例，必须全绿）
...
| 账号A | `~/.boss-copilot/chrome-profile-a` | 9223 | 仅招呼语发送与消息收发 |

注意：挂系统代理（Clash 等）时后端已自动对 localhost CDP 绕过代理。

## 功能与里程碑

| 里程碑 | 功能 | 提交 |
|--------|------|------|
| M1 | SQLite 底座 + xlsx/JSON 导入 + 双账号管理 | 2e59d1d |
...
| M5 | 招呼语生成/队列/发送（账号A UI 级操作 + 全护栏） | 847a127 |
...
5. BOSS 写操作只走账号A、只走 UI 级操作、必须有护栏；spike 脚本放 `tests/spike_*.py`（只读验证用）
...
backend/
  ...
  sender.py      账号A 发送器（UI 级操作 + 护栏 + 熔断）
  chatpoll.py    消息中心（DOM 快照 + AI 草稿 + 点发）
  interview.py   模拟面试 agent
  boss/cdp.py    双账号 CDP 管理（代理绕过）
frontend/        无构建 Vue3（app.js 单文件 + vendored vue.esm）
tests/           60 个单测 + spike 脚本（spike_m5/m6_*）
```

## 已知边界

- 发送/消息依赖账号A 登录态；未登录时相应接口优雅拒绝
