...
# ============================================================
# 采集节奏环境变量（boss-copilot 采集档位注入；缺省=下方原始硬编码值）
# ============================================================
...
def env_range_uniform(min_name, max_name, default_low, default_high):
    """按环境变量区间取随机等待值；未注入时回退原始硬编码区间。"""
    low = _env_float(min_name, default_low)
    high = _env_float(max_name, default_high)
    return random.uniform(min(low, high), max(low, high))


def paced_uniform(low, high):
    """阅读类随机等待；SCRAPER_READ_PAUSE_SCALE 可整体缩放（1.0=原生节奏）。"""
    return random.uniform(low, high) * _env_float("SCRAPER_READ_PAUSE_SCALE", 1.0)
...
def build_company_url(brand_id, page, filters):
    """公司定向岗位列表：web/geek/job SPA 带 brandId 时仍走同一 joblist API。"""
    params = {"brandId": brand_id, "page": page}
...
def scrape_list(keyword, city_input, max_pages, filters, output_path,
                ...
                company_identity=None, dup_stop_ratio=None):
    """抓取搜索或公司定向岗位列表。

    company_identity 非空时进入公司定向模式：值为 brandId 时按
    /web/geek/job?brandId= 参数化翻页；为完整 URL 时首页直接导航，
    若 query 带 brandId 则继续翻页，否则单页结束。
    dup_stop_ratio 为翻页重复早停阈值：某页新增占比 ≤ 阈值时提前结束。
    """
    company_url = None
    company_brand_id = None
    if company_identity:
        city_name, city_code = "定向公司", ""
        if str(company_identity).startswith(("http://", "https://")):
            company_url = str(company_identity)
            parsed = urlparse(company_url)
            brand = dict(parse_qsl(parsed.query)).get("brandId", "")
            if brand:
                company_brand_id = brand
        else:
            company_brand_id = str(company_identity)
    else:
    ...
    try:
        for pg in range(1, max_pages + 1):
            ...
            if pg == 1 and company_url and not company_brand_id:
                # 公司链接无 brandId：整页直接导航，无法参数化翻页
                url = company_url
            elif company_brand_id:
                url = build_company_url(company_brand_id, pg, filters)
            else:
            ...
            # 翻页重复早停（boss-copilot 采集档位）：某页新增占比低于阈值时
            # 继续翻页只会重复爬已见内容，提前结束以省时并降低风控暴露。
            if dup_stop_ratio and pg > 1 and len(jobs) \
                    and (new / len(jobs)) <= dup_stop_ratio:
                print(f"  重复率过高（新增 {new}/{len(jobs)}），提前停止翻页\n")
                break
            ...
            if pg == 1 and company_url and not company_brand_id:
                print("  公司链接未含 brandId，不支持参数化翻页，单页结束\n")
                break

            if pg < max_pages:
                d = env_range_uniform("SCRAPER_LIST_PAGE_GAP_MIN",
...
def scrape_details(list_data, max_details=None, output_path=None,
    ...
    for idx, job in enumerate(jobs):
        ...
        print(f"  加载页面...")
        time.sleep(env_range_uniform("SCRAPER_DETAIL_DWELL_MIN",
                                     "SCRAPER_DETAIL_DWELL_MAX", 5, 10))
        ...
        for i in range(scroll_count):
            ...
            # 有时快滚，有时停下来"阅读"（SCRAPER_READ_PAUSE_SCALE 可整体缩放）
            if random.random() < 0.35:
                time.sleep(paced_uniform(2.0, 5.0))
            else:
                time.sleep(paced_uniform(0.8, 1.8))
        ...
        if random.random() < 0.5:
            ...
            time.sleep(paced_uniform(0.5, 1.5))
        ...
        ws.send("Target.closeTarget", {"targetId": tid})
        ws.close()
        # 详情页间隔加大，随机 10-25 秒（SCRAPER_DETAIL_GAP_* 可覆盖）
        gap = env_range_uniform("SCRAPER_DETAIL_GAP_MIN",
...
示例:
  ...
  %(prog)s --setup-chrome
    ...
    p.add_argument("--company", default=None,
                   help="公司定向采集：brandId 或公司链接 URL；提供后忽略 --keyword/--city")
    p.add_argument("--dup-stop-ratio", type=float, default=None, dest="dup_stop_ratio",
                   help="翻页重复早停阈值 (0-1)：某页新增占比 ≤ 该值时提前结束")
    ...
    if not args.input and not args.company:
        try:
    ...
    if args.input:
        ...
        try:
            list_data = scrape_list(
                ...
                company_identity=args.company,
                dup_stop_ratio=args.dup_stop_ratio,
            )
