...
- `backend/sender.py` 发送器 / `chatpoll.py` 消息中心 / `interview.py` 模拟面试 / `collector.py` 采集执行器（subprocess 调内置引擎）/ `favorites.py` 双账号BOSS收藏同步（推荐页感兴趣Tab只读 + 增量合并）。
- `vendor/boss-zhipin-scraper/`：内置采集引擎（上游 v2.2.0 + 节奏注入/公司定向/翻页早停补丁，见该目录 README），以本仓 venv 解释器直接运行。
...
- 采集引擎 vendor 在 `vendor/boss-zhipin-scraper/`，`BOSS_ZHIPIN_SCRAPER_HOME` 可指向其他引擎副本；升级上游后需按 vendor README 重放补丁。
...
- 数据目录 `~/.boss-copilot/`（可用 `BOSS_COPILOT_HOME` 覆盖）；旧版 `~/.boss-zhipin-scraper/` 的 profile/结果会在启动时幂等迁入。
